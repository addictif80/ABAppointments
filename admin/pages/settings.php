...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'test_cyberpanel') {
    ...
    // Test 1: raw TCP connection to extract host:port
    ...
    if ($decoded) {
        ...
        // Check if response looks like HTML (could be a login page)
    ...
    <div class="card-body">
        <form method="POST">
            ...
            <div class="row g-3">
                ...
                <pre id="testCyberpanelDiag" class="mt-2 p-2 bg-light border rounded small" style="display:none; white-space:pre-wrap; max-height:300px; overflow:auto;"></pre>
            </div>
...
document.getElementById('testCyberpanelBtn')?.addEventListener('click', function() {
    ...
    var diagBox = document.getElementById('testCyberpanelDiag');
    fetch(location.href, {method: 'POST', body: fd}).then(r => r.json()).then(function(data) {
        ...
        if (data.diagnostic) {
            diagBox.textContent = data.diagnostic;
        ...
        diagBox.textContent = 'Erreur JS: ' + e.message;
        diagBox.style.display = 'block';
        btn.disabled = false;
    });
});
